...
export default function ProfileStory({ profile, name, onUpdate }: Props) {
    if (!profile) return null;

    return (
        <ScrollView contentContainerStyle={styles.container}>
            <Text style={styles.title}>Origin Story</Text>
            <Text style={styles.subtitle}>Where does {name} come from?</Text>

            <View style={styles.inputContainer}>
                <TextInput
                    style={styles.input}
                    value={profile.backstory}
                    onChangeText={(text) => onUpdate({ ...profile, backstory: text })}
                    multiline
                    placeholder={`Write the origin of ${name}...`}
